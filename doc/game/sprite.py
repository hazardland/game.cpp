...
class Sprite:
    ...
    def frame(self, index):
        """
            Returns frame cell and row based on frame index
            Considers self.vertical=False/True parameter
        """
        if self.vertical:
            cell = ((index-1) // self.rows) + 1
            row = index - (cell-1) * self.rows
        else:
            row = ((index-1) // self.cells) + 1
            cell = index - (row-1) * self.cells
